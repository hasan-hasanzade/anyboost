...
const Calculator: React.FC<CalculatorProps> = ({ type, inputValue, count, elo, currentRating, desiredRating, images }) => {
  ...
  const [premierCalibrationCount, setPremierCalibrationCount] = useState<number>(0);
  ...
  const calculatePrice = () => {
    ...
    if (type === 'premier') price += Math.max(0, desiredElo - currentElo) / 200 * 500;
  ...
  const handleEloChange = (e: ChangeEvent<HTMLInputElement>, setElo: React.Dispatch<React.SetStateAction<number>>) => {
  ...
  const incrementPremierCalibrationCount = (increment: number) => {
    setPremierCalibrationCount((prevCount) => Math.max(prevCount + increment, 0));
  };

  const incrementEloIndex = (setIndex: React.Dispatch<React.SetStateAction<number>>, index: number) => {
    if (Array.isArray(elo)) {
      setIndex((prevIndex) => (prevIndex + 1) % elo.length);
    }
  };

  const decrementEloIndex = (setIndex: React.Dispatch<React.SetStateAction<number>>, index: number) => {
    if (Array.isArray(elo)) {
  ...
  const currentImage = Array.isArray(images) && images.length > 0 ? images[currentEloIndex] : null;

  const desiredEloDisplay = Array.isArray(elo) ? elo[desiredEloIndex] : elo;
  const desiredImage = Array.isArray(images) && images.length > 0 ? images[desiredEloIndex] : null;

  return (
    <div className={styles.body}>
      <div className={styles.calcs}>
        <div className={styles.item}>
          {currentImage && <Image src={currentImage} width={32} height={100} alt='звания' />}
          <div className={styles.currentRating}>
            <div className={styles.currentCalc}>
              <button className={styles.subtract} onClick={() => type === 'premier' ? incrementElo(-200) : decrementEloIndex(setCurrentEloIndex, currentEloIndex)}>-{count}</button>
              ...
              <button className={styles.add} onClick={() => type === 'premier' ? incrementElo(200) : incrementEloIndex(setCurrentEloIndex, currentEloIndex)}>+{count}</button>
            </div>
          </div>
        </div>
        ...
        <div className={styles.item}>
          <div className={styles.desiredRating}>
            <div className={styles.desiredCalc}>
              <button className={styles.subtract} onClick={() => type === 'premier_calibration' ? incrementPremierCalibrationCount(-1) : incrementDesiredElo(-200)}>-{count}</button>
              <div className={styles.center}>
              ...
              <button className={styles.add} onClick={() => type === 'premier_calibration' ? incrementPremierCalibrationCount(1) : incrementDesiredElo(200)}>+{count}</button>
            </div>
          </div>
          {type !== 'premier_calibration' && desiredImage && <Image src={desiredImage} width={32} height={100} alt='звания' />}
        </div>
      </div>
